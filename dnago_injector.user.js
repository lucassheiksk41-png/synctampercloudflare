...
// @version      6.3
// @description  Troca Automática de URL (Bob vs IBOs) + Bypass
// @author       Você & Omini
// @match        *://cms.omini.fit/*
// @match        *://*.omini.fit/*
// @match        *://*.dna-go.com/*
...
// @match        *://*.iboplayer.com/*
// @match        *://*.iboproapp.com/*
// @updateURL    https://raw.githubusercontent.com/lucassheiksk41-png/synctampercloudflare/main/dnago_injector.user.js
// @downloadURL  https://raw.githubusercontent.com/lucassheiksk41-png/synctampercloudflare/main/dnago_injector.user.js
// @grant        GM_setValue
// @grant        GM_getValue
// ==/UserScript==
...
    const OUTROS_DNS = "http://bttlinkib.sbs:8880";
    const OUTROS_NOME = "LISTA 1 - BTV";
...
                
                if (inputs.length >= 2) {
                    let macInput = inputs[0]; let keyInput = inputs[1];
...
    // =========================================================================
    console.log("🟣 OMINI HUB DNA-GO: Script iniciado (Versão 6.3 - Troca Automática)...");
...
        if (!document.body || document.getElementById('omini-wrapper-dnago')) return;

        const wrapper = document.createElement('div');
        wrapper.id = 'omini-wrapper-dnago';
...
        painel.id = 'omini-hub-panel-dnago';
...
        style.innerHTML = `#omini-hub-panel-dnago::-webkit-scrollbar { width: 3px; } #omini-hub-panel-dnago::-webkit-scrollbar-track { background: #0f172a; } #omini-hub-panel-dnago::-webkit-scrollbar-thumb { background: #f97316; } #omini-hub-panel-dnago input { outline: none; } #omini-hub-panel-dnago input:focus { border-color: #f97316 !important; }`;
...
                <span>🧬 DNA-GO HUB v6.3 (CF PAID)</span>
            </div>
...
                        <input type="text" id="hub-nome1" style="flex: 2; background: #1e293b; color: white; border: 1px solid #334155; padding: 4px 6px; border-radius: 4px; font-size: 10px;">
                        <input type="text" id="hub-fmt1" style="flex: 1; background: #1e293b; color: white; border: 1px solid #334155; padding: 4px 6px; border-radius: 4px; font-size: 10px;">
                    </div>
                </div>
...
        bolha.id = 'omini-hub-bolha-dnago';
...
            } else {
                painel.style.display = 'none'; bolha.innerHTML = `<span style="font-size: 20px; filter: drop-shadow(0 0 6px #f97316);">🧬</span>`; bolha.style.background = `linear-gradient(135deg, #2e1065, #6b21a8)`; bolha.style.borderColor = `#f97316`;
            }
        };
...
            
            // FUNÇÃO MÁGICA: Preenche os campos baseado no app selecionado
            const aplicarPadrao = (appMode) => {
                if (appMode === 'BOB') {
                    document.getElementById('hub-dns1').value = BOB_DNS;
                    document.getElementById('hub-nome1').value = BOB_NOME;
                    document.getElementById('hub-fmt1').value = BOB_FMT;
                } else {
                    document.getElementById('hub-dns1').value = OUTROS_DNS;
                    document.getElementById('hub-nome1').value = OUTROS_NOME;
                    document.getElementById('hub-fmt1').value = OUTROS_FMT;
                }
            };

            // Puxa o último app usado e já aplica a configuração correta
            let appSalvo = GM_getValue('hub_last_app', 'BOB');
            document.getElementById('hub-app').value = appSalvo;
            aplicarPadrao(appSalvo);

            // Escuta as mudanças no seletor e troca os links na hora
            document.getElementById('hub-app').addEventListener('change', (e) => {
                aplicarPadrao(e.target.value);
            });
            
            document.getElementById('hub-dns2').value = GM_getValue('hub_dns2', ''); 
            document.getElementById('hub-nome2').value = GM_getValue('hub_nome2', 'Backup VIP'); 
            document.getElementById('hub-fmt2').value = GM_getValue('hub_fmt2', 'ts');
            
...
            document.getElementById('hub-mac').value = ''; document.getElementById('hub-key').value = '';
            document.getElementById('hub-user').value = ''; document.getElementById('hub-pass').value = '';

            let textoPaginaInteira = document.body.innerText || "";
            let matchUserDNA = textoPaginaInteira.match(/Usuário\s*\n\s*([a-zA-Z0-9]+)/i) || textoPaginaInteira.match(/Usuário[:\-\s]+([a-zA-Z0-9]+)/i);
            if (matchUserDNA) document.getElementById('hub-user').value = matchUserDNA[1].trim();
            let matchPassDNA = textoPaginaInteira.match(/Senha\s*\n\s*([a-zA-Z0-9]+)/i) || textoPaginaInteira.match(/Senha[:\-\s]+([a-zA-Z0-9]+)/i);
            if (matchPassDNA) document.getElementById('hub-pass').value = matchPassDNA[1].trim();
...
            btn.disabled = true;
            statusEl.innerHTML = "<b style='color:#f97316;'>🔄 Baixando (5 Captchas Pentakill)...</b>";
            try {
                let promises = [];
...
                const payload = {
                    app: app, mac: mac, key: key, user: user, pass: pass, apikey: API_2CAPTCHA,
                    captchas: captchas_gerados, apagar_antigas: apagarTudo,
                    srv1_ativo: true, srv1_dns: dns1, srv1_fmt: document.getElementById('hub-fmt1').value, srv1_nome: document.getElementById('hub-nome1').value,
                    srv2_ativo: document.getElementById('hub-chk-dns2').checked, srv2_dns: document.getElementById('hub-dns2').value, srv2_fmt: document.getElementById('hub-fmt2').value, srv2_nome: document.getElementById('hub-nome2').value
                };
...
            const appSel = document.getElementById('hub-app').value;
            const workerBase = appSel === 'IBOPRO' ? WORKER_ESCRAVO : WORKER_MOTOR;
            try {
...
    }
    setInterval(() => { if (!document.getElementById('omini-wrapper-dnago')) criarPainelHub(); }, 1000);
})();
